...
import cellsSlice, { Item_T } from '../../../state/reducers/CellsReducer'
...
const Cell = ({ cell_id }: { cell_id: string }) => {
    ...
    let Element: undefined | Item_T = undefined

    if (cell?.item_id) {
        let item = items.find(el => el.id === cell?.item_id)
        Element = item
    }
    ...
    const addImageToBoard = (item_id: string, old_cell_id: string) => {
        ...
        if(item) {
            let data = {id: cell_id, item_id: item.id, old_cell_id}
            if (data) {
